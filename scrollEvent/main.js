...
var now = 1;
 ...
 // 스크롤바 이벤트
 // 왼쪽에 적힌 window가 'scroll' 될떄마다 함수를 실행
 window.addEventListener('scroll', function(){
  // 전체창 스크롤 상태는 scrollY 사용
 ...
 document.querySelector('.lorem').addEventListener('scroll', function(){
  // 현재 스크롤 상태는 scrollTop을 사용
  var now_scroll = document.querySelector('.lorem').scrollTop;
  // 화면에 보이는 div 박스 크기는 clientHeight 사용 
  var now_box_size = document.querySelector('.lorem').clientHeight;
  // 전체 스크롤바 div 박스 크기는 scrollHeight를 사용
  var total_box_size = document.querySelector('.lorem').scrollHeight;

// (현재 스크롤 상태 + 화면에 보이는 div 박스 크기 == 전체 스크롤바 div 크기)
  if (now_scroll + now_box_size == total_box_size){
    alert('약관을 모두 읽으셨습니다.');
  }
 })
